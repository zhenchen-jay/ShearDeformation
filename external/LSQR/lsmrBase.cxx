...
void
lsmrBase::SetMaximumNumberOfIterations(int value )
{
  this->itnlim = value;
}
...
void
lsmrBase::Scale( int n, double factor, double *x ) const
{
  double * xend = x + n;
  while( x != xend )
    {
...
double
lsmrBase::Dnrm2( int n, const double *x ) const
{
 ...
 */
void lsmrBase::
Solve( int m, int n, const double * b, double * x )
{
  const double zero = 0.0;
  ...
  int localVecs = std::min( localSize, std::min( m,n ) );
  ...
  do {
	...
	this->Aprod2( m, n, v, u );    // v = A'*u
	if ( localOrtho ) {
	  int localOrthoLimit = localVQueueFull ? localVecs : localPointer+1;

	  for( int localOrthoCount =0; localOrthoCount<localOrthoLimit;
	       ++localOrthoCount) {
